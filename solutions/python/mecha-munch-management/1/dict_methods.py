...
def update_recipes(ideas: dict[str, dict[str, int]], recipe_updates: tuple[tuple[str, dict[str, int]], ...]):
...
def send_to_store(cart: dict[str, int], aisle_mapping: dict[str, list]) -> dict[str, list]:
    ...
    return {item: [cart[item]] + aisle_mapping[item] for item in sorted(cart.keys(), reverse=True)}
